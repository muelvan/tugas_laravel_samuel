...
<div class="max-w-3xl mx-auto p-6 bg-white rounded-2xl shadow-md mt-8">
    ...
    <form action="{{ isset($siswa) ? route('siswas.update', $siswa->id) : route('siswas.store') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="nama" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" value="{{ $siswa->nama ?? old('nama') }}" required>
        </div>

        <div>
            <label class="block mb-1 text-sm font-medium text-gray-700">NIS</label>
            <input type="text" name="nis" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" value="{{ $siswa->nis ?? old('nis') }}" required>
        </div>

        <div>
            <label class="block mb-1 text-sm font-medium text-gray-700">Alamat</label>
            <textarea name="alamat" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" required>{{ $siswa->alamat ?? old('alamat') }}</textarea>
        </div>

        <div>
            <label class="block mb-1 text-sm font-medium text-gray-700">Jenis Kelamin</label>
            <select name="jenis_kelamin" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" required>
        ...
        <div>
            ...
            <input type="date" name="tanggal_lahir" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500" value="{{ $siswa->tanggal_lahir ?? old('tanggal_lahir') }}" required>
        </div>
